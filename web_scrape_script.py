...
def find_links_until_sub(td):
    links = []
    for elem in td.descendants:
        if isinstance(elem, Tag):
            if elem.name == "sup":
                # once a <sub> tag is encountered, break out of the loop.
                break
            if elem.name == "a":
                links.append(elem)
    return links
...
def scrape_person_list(person_list):
    for person in person_list:
        ...
        # Use a flexible selector: the infobox should contain both 'infobox' and 'vcard'
                ...
                if header:
                    ...
                    if "Born" in header_text:
                        ...
                        if birthplace_div:
                            ...
                            # remove parenthesized content (like age).
                            ...
                            if parts:
                                # take the last element (after the last comma) as the birth country.
        ...
        if header:
            ...
            if "Hosted by" in header_text:
                ...
                if event_host:
                    ...
                    birth_date, birth_country, death_date = scrape_person_list(event_host)
                ...
                if li_items:
                    for li in li_items:
                        ...
                        if formatted_host:  # Ensure it's not an empty list
            ...
            if "Produced by" in header_text:
                ...
                if li_items:
                    ...
                    if links:
                        ...
                        event_producer = [format_host(td.text.strip())]
                if event_producer:
